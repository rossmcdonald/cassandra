...
{% if cassandra_max_heap_size != None %}
MAX_HEAP_SIZE="{{ cassandra_max_heap_size }}"
{% endif %}
{% if cassandra_heap_newsize != None %}
HEAP_NEWSIZE="{{ cassandra_heap_newsize }}"
{% endif %}
{% if cassandra_malloc_arena_max != None %}
MALLOC_ARENA_MAX="{{ cassandra_malloc_arena_max }}"
{% endif %}
...
{% if cassandra_enable_assertions %}
...
JVM_OPTS="$JVM_OPTS -Xss{{ cassandra_per_thread_stack_size }}"

{% for option in cassandra_jvm_options %}
JVM_OPTS="$JVM_OPTS -XX:{{ option }}"
{% endfor %}
...
{% if cassandra_enable_gc_logging %}
